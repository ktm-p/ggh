...
class GGH:
    ...
    def generate_private_key(self) -> fl.fmpz_mat:
        ...
        return Utils.conv_to_fmpz(basis)

    def generate_private_key_rectangular(self) -> fl.fmpz_mat:
        k = fl.fmpz(self.thresh * math.ceil(math.sqrt(self.dimension)))
        identity = Utils.conv_to_fmpz(np.eye(self.dimension, dtype=int))
        basis = None
        while not Utils.check_basis(basis):
            r = Utils.conv_to_fmpz(np.random.choice(np.array([-self.thresh, self.thresh]), size=(self.dimension, self.dimension)))
            basis = r + (k * identity)
