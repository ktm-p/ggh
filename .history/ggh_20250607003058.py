...
class GGH:
    ...
    def generate_private_key(self) -> np.array:
        ...
        while (not Utils.check_basis(self.dimension, basis)) or (Utils.hadamard_ratio(self.dimension, basis) < 0.75):
            basis = np.random.choice(np.array([-self.thresh, self.thresh]), size=(self.dimension, self.dimension)).astype(int)
...
message = "hello! how are you? i am very good! and you are? hahaha, i am glad! i might be crazy. hi."
dim = len(message)
print("Dimension:", dim)
