...
np.random.seed(69)
class GGH:
    def __init__(self, dimension:int=2, sigma:int=3, thresh:int=100):
    ...
    def generate_private_key(self) -> np.array:
        ...
        while (not Utils.check_basis(self.dimension, basis)) or (Utils.hadamard_ratio(self.dimension, basis) < 0.75):
            # Constructs random vector v with entries in [-thresh, thresh].
            vecs = []
...
def generate_private_key(dimension, thresh) -> np.array:
    basis = None
    # Checks if set of vectors is a basis and has a sufficiently high Hadamard Ratio
    while (not Utils.check_basis(dimension, basis)) or (Utils.hadamard_ratio(dimension, basis) < 0.75):
        # Constructs random vector v with entries in [-thresh, thresh].
        basis = np.random.choice(np.arange(-thresh, thresh, 1), size=(dimension, dimension))

    return basis

def generate_private_key_old(dimension, thresh) -> np.array:
        basis = None
        # Checks if set of vectors is a basis and has a sufficiently high Hadamard Ratio
        while (not Utils.check_basis(dimension, basis)) or (Utils.hadamard_ratio(dimension, basis) < 0.75):
            # Constructs random vector v with entries in [-thresh, thresh].
...
new_start = time.time()
t = generate_private_key(dim, 100)
new_end = time.time()
print(t)

old_start = time.time()
t_old = generate_private_key_old(dim, 100)
old_end = time.time()
print(t_old)

print("New:", new_end - new_start)
print("Old:", old_end - old_start)