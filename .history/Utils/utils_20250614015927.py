...
class Utils:
    ...
    def np_to_flq(array: np.ndarray):
        fractions = [[fl.Fraction(item) for item in row] for row in array.tolist()]
        return fl.fmpq_mat([[fl.fmpq(f.numerator, f.denominator) for f in row] for row in fractions])

    def babai_round(basis: fl.fmpz_mat, basis_inv:fl.fmpz_mat, vec: fl.fmpz_mat) -> fl.fmpz_mat:
        x = vec * basis_inv
