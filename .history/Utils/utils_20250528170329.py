...
class Utils:
    ...
    def generate_private_key(dimension:int, thresh:int) -> np.array:
        basis = None
        while (not Utils.check_basis(dimension, basis)) or (Utils.hadamard_ratio(dimension, basis) < 0.7):
            vecs = []
    ...
    def generate_error(dimension:int, sigma: int):
        return np.random.choice(np.array([-sigma, sigma]), size=(dimension, 1))
...
print(Utils.generate_error(5, 1))